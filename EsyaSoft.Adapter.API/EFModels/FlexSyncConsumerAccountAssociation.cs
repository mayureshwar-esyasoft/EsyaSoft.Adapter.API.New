using System;
using System.Collections.Generic;

namespace EsyaSoft.Adapter.API.EFModels;

public partial class FlexSyncConsumerAccountAssociation
{
    public long FlexSyncConsumerAccountAssociationId { get; set; }

    public Guid FlexSyncConsumerAccountAssociationAltId { get; set; }

    public Guid FlexSyncHeaderAltId { get; set; }

    public string? ConsumerId { get; set; }

    public string? ConsumerDescription { get; set; }

    public Guid? FlexSyncConsumerAltId { get; set; }

    public string? AccountId { get; set; }

    public string? AccountDescription { get; set; }

    public Guid? FlexSyncAccountAltId { get; set; }

    public string? Status { get; set; }

    public string? RelType { get; set; }

    public DateTime CreatedOn { get; set; }

    public DateTime UpdatedOn { get; set; }

    public string CreatedBy { get; set; } = null!;

    public string UpdatedBy { get; set; } = null!;
}
